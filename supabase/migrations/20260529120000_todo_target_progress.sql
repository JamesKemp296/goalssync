-- Per-todo repeat targets: increment progress_count until target_count, then is_complete.
...
begin
  if not exists (
    select 1 from pg_constraint
     where conname = 'todos_progress_lte_target'
       and conrelid = 'public.todos'::regclass
  ) then
    alter table public.todos
      add constraint todos_progress_lte_target
        check (progress_count <= target_count);
  end if;
end $$;

update public.todos
   set progress_count = case when is_complete then target_count else 0 end;
